...
This document lives under **new-tools-schema** and aligns with [`newtoolschema.json`](newtoolschema.json), [`resolver-architecture.md`](resolver-architecture.md), and the optimizations in [`../optimized-tools-catalog-pitch.md`](../optimized-tools-catalog-pitch.md) and [`../tools-catalog-settings-unification.md`](../tools-catalog-settings-unification.md). The **canonical prompt chunk** the harness may assemble is still under [`../../../Plugins/UnrealAiEditor/prompts/chunks/04-tool-calling-contract.md`](../../../Plugins/UnrealAiEditor/prompts/chunks/04-tool-calling-contract.md); keep substantive edits in sync or note deltas here.

---

## Naming: `get_` / `set_` / `query_` / `apply_` (mental model)

Use this pattern to choose tools **consistently** and keep **narrow context**: discover → read → write → verify.

| Pattern | Role | Examples (current catalog `tool_id`s) |
|--------|------|----------------------------------------|
| **`*_query` / `*_get`*** | Read-only, idempotent | `asset_registry_query`, `editor_get_selection`, `viewport_camera_get_transform`, `editor_state_snapshot_read`, `blueprint_get_graph_summary`, `material_get_usage_summary`, `pie_status` |
| **`*_set` / `*_apply`*** | Mutations | `editor_set_selection`, `actor_set_transform`, `viewport_set_view_mode`, `viewport_camera_set_transform`, `asset_apply_properties`, `blueprint_apply_ir`, `project_file_write_text` |
| **Search / fuzzy** | Discovery before precise paths | `scene_fuzzy_search`, `asset_index_fuzzy_search`, `source_search_symbol` |
| **Imperative verbs** | One-shot actions (not disguised as settings) | `blueprint_compile`, `pie_start`, `asset_open_editor`, `asset_save_packages`, `asset_create` |
| **Future: `setting_query` / `setting_apply`** | Unified key/value for **persistent or session** config | Envelope in [`newtoolschema.json`](newtoolschema.json) (`SettingQueryEnvelope`, `SettingApplyEnvelope`); **not** for Blueprint graphs, actor CRUD, or captures — see settings-unification doc |

**Pairing rule:** Where a read has an obvious write, the tool **summary** in the catalog should **name the paired tool** in one sentence so one-line BM25 hits still teach the contract (per optimized-tools pitch).

**Poor fit for generic settings:** Actor transforms, attachments, Blueprint IR, sequencer, one-shot screenshots — stay **specialized tools** with their own schemas.

---

## Schema and retrieval (optimizations)

- **Canonical JSON keys:** Use **one name per concept** in arguments (`object_path`, `actor_path`, `relative_path`). Prefer **no duplicate aliases** in the published schema; repair fallbacks may live in the **dispatch/handler** layer (see pitch §3.3).
- **BM25:** Indexed text is `tool_id`, `summary`, `category`, `tags`, `tool_surface.domain_tags` — tune **summaries** and **tags** for user language and query-shaper vocabulary, not parameter names.
- **Tiered appendix:** Only top-ranked tools get long parameter excerpts; keep each tool’s `parameters` **compact** (`additionalProperties: false`, small enums).
- **Core pack:** `always_include_in_core_pack` only for **small, safe, high-frequency** tools so guardrails do not crowd out expanded slots.

---

## Plan mode and tools

- **Plan mode — planner pass:** The model profile uses **no callable tools** for the DAG-only planner turn. Output **`unreal_ai.plan_dag` JSON only**; do not emit `tool_calls` (see `09-plan-dag.md` in prompts). Node execution uses normal Agent tool calling in separate turns.
...
- **Read-vs-mutate routing pairs (apply strictly):**
  ...
  - `material_get_usage_summary` (read usage intel) vs `material_instance_set_scalar_parameter` / `material_instance_set_vector_parameter` (write parameter value).
...
For **gameplay Blueprint** work (movement, overlap, timers, health, spawners), read **`10-mvp-gameplay-and-tooling.md`** in prompts. Prefer **`blueprint_export_ir` → `blueprint_apply_ir` → `blueprint_compile`** (set **`format_graphs: true`** on compile when every script graph should get a formatter pass), **`asset_create` + `asset_apply_properties`**, then **`pie_start`** / **`pie_stop`** to verify. Empty `{}` in automated tests often yields `ok:false` on write tools until required paths are filled — use search/selection tools first.
