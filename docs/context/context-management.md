...
**Tool surface (orthogonal):** narrowing or tiering **which tools appear in the LLM tool index** (`unreal_ai_dispatch` + markdown, or native `tools[]`) is handled by the **tool surface pipeline** in `UnrealAiToolSurfacePipeline` / `UnrealAiTurnLlmRequestBuilder`, not by `BuildContextWindow`. It uses a **separate** lexical (BM25) index over catalog text and editor snapshot hints for **domain bias** — not the SQLite vector store used for docs/snippets. **Blueprint Builder** turns (`bBlueprintBuilderTurn`) rebuild BM25 over a **narrow corpus** (`UnrealAiBlueprintToolGate::PassesBuilderNarrowBm25Corpus`: tools flagged `blueprint_builder_core`, `category == blueprints`, or a small utility allowlist). Tiered-surface telemetry exposes `surface_profile` **`blueprint_builder_narrow_bm25`** on those turns. After a builder result, **`bInjectBlueprintBuilderResumeChunk`** injects **`chunks/13-blueprint-builder-resume.md`** into the main-agent system prompt for exactly one LLM round. See [`tools-expansion.md`](../tooling/tools-expansion.md) and the **`tool-surface-graph`** view in [`architecture-maps/architecture.dsl`](../architecture-maps/architecture.dsl).
