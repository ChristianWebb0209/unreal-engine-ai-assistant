...
Tools **removed** from the catalog but still routed in `UnrealAiToolDispatch.cpp` return **`status: tool_deprecated`** and a **`suggested_correct_call`** toward **`blueprint_graph_patch`** / introspect — do not document them as active agent tools.
