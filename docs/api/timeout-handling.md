...
| **Plan mode segments** | Plan runs **planner** then **per-node** agent work. Each **sub-turn** resets the sync window; **overall** wall across segments is optional. | Per segment: same `HarnessSyncWaitMs`; total plan wall: `HarnessPlanMaxWallMs` (0 = off) | `UnrealAiHarnessScenarioRunner.cpp`, `FUnrealAiPlanExecutor` |
