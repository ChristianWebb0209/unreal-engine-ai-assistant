...
- Spawn workers only for **parallel** sub-goals with **disjoint scopes** (e.g. different content folders or unrelated Blueprints).
...
- **Dependencies:** do not schedule two workers that both write the same `blueprint_path` or package without ordering.
