...
- **Main Agent vs Blueprint Builder:** In normal **Agent** turns, graph-mutation tools such as `blueprint_graph_patch`, `blueprint_format_graph`, `blueprint_format_selection`, `blueprint_compile`, and `blueprint_add_variable` are withheld from the tool roster. Delegate substantive Blueprint graph work with `<unreal_ai_build_blueprint>...</unreal_ai_build_blueprint>` per **`12-build-blueprint-delegation.md`**. Automated **Blueprint Builder** sub-turns receive the full tool surface and the prompts under `chunks/blueprint-builder/`.
...
- **UE 5.7 Blueprint scope (this plugin):** Target **Unreal Engine 5.7** graphs only. When you have graph-mutation tools (Blueprint Builder sub-turns), implement **as much Blueprint complexity as the user request needs**—multiple graphs, custom events, Enhanced Input, state machines, branches, latent nodes, component calls, etc. **Do not** default to “minimal” graphs to avoid work. For **arbitrary** node types and wiring, use **`blueprint_graph_patch`**. Use **`blueprint_apply_ir`** when the compact op set fits; fall back to **`blueprint_graph_patch`** when IR ops or merge rules are insufficient. Ground pin names in **`blueprint_export_ir`**, **`blueprint_get_graph_summary`**, and engine tool errors—not memorized docs. On **main Agent** turns without those tools, delegate graph work via **`12-build-blueprint-delegation.md`** instead of improvising mutations.
...
- **Blueprint assets vs class defaults:** **`asset_export_properties`** / **`asset_apply_properties`** operate on the **`UBlueprint` asset object** (editor fields on that UObject). They do **not** read or write **inherited component defaults** on the generated class (e.g. **`CharacterMovement.JumpZVelocity`** on a Character Blueprint). For scalar tuning on a named Actor component’s class default, use **`blueprint_set_component_default`** (`blueprint_path`, `component`, `property`, `value`) instead of nesting keys under `properties`. For logic in graphs, use **`blueprint_export_ir`** for inspection, then **`blueprint_graph_patch`** for arbitrary nodes/wiring and/or **`blueprint_apply_ir`** when the compact IR surface is enough.
...
For **gameplay Blueprint** work (movement, overlap, timers, health, spawners), read **`10-mvp-gameplay-and-tooling.md`**. Prefer **`blueprint_export_ir`** first; then **`blueprint_graph_patch`** for arbitrary nodes/wiring or **`blueprint_apply_ir`** when the compact IR op set fits; finish with **`blueprint_compile`** (set **`format_graphs: true`** when every script graph should get a formatter pass). For **component class defaults** (e.g. jump height on **`CharacterMovement`**), prefer **`blueprint_set_component_default`** over **`asset_apply_properties`**. For generic non-Blueprint assets, use **`asset_create` + `asset_apply_properties`**. Then **`pie_start`** / **`pie_stop`** to verify. Empty `{}` in automated tests often yields `ok:false` on write tools until required paths are filled — use search/selection tools first.
