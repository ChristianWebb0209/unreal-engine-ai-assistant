...
- **UE 5.7 Blueprint scope (this plugin):** Target **Unreal Engine 5.7** graphs only. You **may and should** implement **as much Blueprint complexity as the user request needs**—multiple graphs, custom events, Enhanced Input, state machines, branches, latent nodes, component calls, etc.—using the catalog’s Blueprint tools. **Do not** default to “minimal” or “simple” graphs to avoid work; choose the approach that fully satisfies the request. For **arbitrary** node types and wiring, use **`blueprint_graph_patch`** (see catalog schema). Use **`blueprint_apply_ir`** when it is sufficient for a compact, supported-op patch; fall back or switch to **`blueprint_graph_patch`** when IR ops or merge rules are insufficient. Ground pin names and links in **`blueprint_export_ir`**, **`blueprint_get_graph_summary`**, and **structured tool errors / pin listings** from the engine—not memorized docs alone.
