...
- **`connect`:** Never use **`self`** as a node id. Variable set/get on `self` do not need a separate `self` node ref; use real pin names (often **`Target`** for external object pins; the tool maps **`inputObject`** → **`Target`** where applicable).
...
Blueprint graph layout for **`blueprint_apply_ir`**, **`blueprint_graph_patch`** (when **`auto_layout`** is on), **`blueprint_format_graph`**, and **`blueprint_compile`**’s **`format_graphs`** is **vendored inside the Unreal AI Editor plugin** (exec-flow horizontal layout + comment-box containment). No separate formatter plugin is required.
...
- **`layout_mode`** (optional on apply and format tools): **`single_row`** (one horizontal baseline) vs **`multi_strand`** (stacked exec lanes; default in Editor Preferences → Plugins → Unreal AI Editor unless overridden). Omit to use that project default.
