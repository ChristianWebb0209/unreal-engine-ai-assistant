...
- **Catalog note:** In **`UnrealAiToolCatalog.json`**, **`blueprint_graph_patch`**, **`blueprint_compile`**, **`blueprint_format_graph`**, and related graph mutators are **`agent_surfaces: [\"blueprint_builder\"]`** only—they **do not** appear on the default main Agent roster. The main agent should **not** plan on calling them; use **`<unreal_ai_build_blueprint>`** for patch/compile/format work after assets exist. (Power-user sessions that disable surface gating are the exception.)
