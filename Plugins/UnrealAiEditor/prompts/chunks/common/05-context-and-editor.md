...
**Stale or missing snapshot:** call `editor_state_snapshot_read` or use `editor_get_selection` / `viewport_list_visible_actors` / `scene_fuzzy_search` / registry search **before** transforms, spawns, destructive ops, or writes that depend on “what exists now.” **Selection, viewport framing, and Content Browser sync** all need **concrete paths** from context or discovery—do not assume them without reading. Selection tools address **actors in the level**, not Content Browser assets—see **`04-tool-calling-contract.md`** (selection vs assets vs viewport).
