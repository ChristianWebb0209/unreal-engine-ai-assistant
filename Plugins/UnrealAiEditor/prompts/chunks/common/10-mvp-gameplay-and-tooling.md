...
- **When graph mutation tools are listed:** for **writes**, use **`blueprint_graph_patch`** for all Kismet nodes and wiring (`semantic_kind` or `k2_class`, explicit **`connect`**). When user policy is C++-only, skip Blueprint writes. New Blueprint assets: **`asset_create`** with correct factory / **`parent_class`**, then patch graphs. Keep **`auto_layout`** on (default) and choose **`layout_scope`** (`patched_nodes`, **`patched_nodes_and_downstream_exec`** after splices/interrupts on exec chains—union capped at **384** nodes; check **`layout_downstream_truncated`** / **`layout_warnings`** if huge, or **`full_graph`** for explicit whole-graph tidy) and optional **`layout_anchor`** (`neighbor` vs **`below_existing`**) per catalog. Follow with **`blueprint_compile`** / **`blueprint_verify_graph`** when available; use **`format_graphs: true`** or **`blueprint_format_graph`** (`format_scope` `full_graph` or `selection`) when listed. Split large graph work into **several** patches or use **`ops_json_path`**. For **`K2Node_CallFunction`**, set **`class_path` + `function_name`** on a **`create_node`** op (or **`semantic_kind`** **`call_library_function`**)—there is **no** top-level **`op`:** **`call_function`** (see **`04`** and **`07-graph-patch-canonical.md`**).
