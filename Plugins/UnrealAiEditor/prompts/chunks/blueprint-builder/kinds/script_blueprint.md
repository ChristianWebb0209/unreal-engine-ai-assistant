...
- After substantive edits: **`blueprint_compile`** then **`blueprint_verify_graph`** with steps such as **`links`**, **`orphan_pins`**, **`duplicate_node_guids`**, **`dead_exec_outputs`**, **`pin_type_mismatch`** — **fix and retry** until clean or return blocked (fail-safe chunk). Use **`blueprint_format_graph`** / **`blueprint_format_selection`** when layout matters; adjust spacing / preserve-existing / knots in **Editor Preferences → Unreal AI Editor → Blueprint Formatting** (or the Blueprint toolbar AI format options) rather than tool arguments.
