...
- **Wiring is never implicit:** any batch that **creates** nodes must also **`connect`** them in the same **`blueprint_graph_patch`** (or use `break_link` / `splice_on_link` as needed). Do not ship create-only patches and expect edges to appear.
