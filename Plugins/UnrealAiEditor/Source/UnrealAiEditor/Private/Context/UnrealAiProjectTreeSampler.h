...
namespace UnrealAiProjectTreeSampler
{
	...
	bool MaybeRefreshProjectTreeSummary(const FString& ProjectId, FProjectTreeSummary& InOutSummary, bool bForceRefresh = false);
	/** Shared per-project cache used by callers outside context-service instance boundaries. */
	const FProjectTreeSummary& GetOrRefreshProjectSummary(const FString& ProjectId, bool bForceRefresh = false);
