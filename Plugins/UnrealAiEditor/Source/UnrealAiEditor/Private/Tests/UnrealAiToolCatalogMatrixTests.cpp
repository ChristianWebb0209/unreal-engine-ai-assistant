...
namespace UnrealAiToolCatalogMatrixTestsPriv
{
...
bool FUnrealAiToolCatalogMatrixTest::RunTest(const FString& Parameters)
{
	...
	FString GitCommit = FPlatformMisc::GetEnvironmentVariable(TEXT("GIT_COMMIT"));

	FUnrealAiToolCatalog Cat;
	TestTrue(TEXT("catalog LoadFromPlugin"), Cat.LoadFromPlugin());
	if (!Cat.IsLoaded())
	{
		return false;
	}
	...
	Cat.ForEachTool([&](const FString& ToolId, const TSharedPtr<FJsonObject>& Def) {
		...
		if (UnrealAiToolCatalogMatrixTestsPriv::IsBannedTool(Def))
		{
			++SkippedBanned;
			TSharedPtr<FJsonObject> Row = MakeShared<FJsonObject>();
			Row->SetStringField(TEXT("tool_id"), ToolId);
		...
		FString FixtureErr;
		if (!UnrealAiToolCatalogMatrixTestsPriv::TryLoadFixtureArgs(ToolId, Args, FixtureErr))
		{
			UnrealAiToolCatalogMatrixTestsPriv::AppendViolation(ContractViolations, ToolId, FixtureErr);
			TSharedPtr<FJsonObject> Row = MakeShared<FJsonObject>();
			Row->SetStringField(TEXT("tool_id"), ToolId);
		...
		{
			UnrealAiToolCatalogMatrixTestsPriv::AppendViolation(ContractViolations, ToolId, TEXT("empty ContentForModel"));
		...
		TSharedPtr<FJsonObject> ParsedBody;
		{
			...
			{
				UnrealAiToolCatalogMatrixTestsPriv::AppendViolation(ContractViolations, ToolId, TEXT("ContentForModel not valid JSON"));
				Row->SetStringField(TEXT("tier"), TEXT("contract_fail"));
				ResultRows.Add(MakeShared<FJsonValueObject>(Row.ToSharedRef()));
				return;
			}
		...
		if (!UnrealAiToolCatalogMatrixTestsPriv::ResponseLooksStructured(ParsedBody))
		{
			UnrealAiToolCatalogMatrixTestsPriv::AppendViolation(
				ContractViolations,
				ToolId,
				TEXT("JSON missing ok/status/error"));
			Row->SetStringField(TEXT("tier"), TEXT("contract_fail"));
		}
		else
		{
	...
	FString OutStr;
	{
		...
		{
			AddError(TEXT("Failed to serialize tool_matrix_last.json"));
			return false;
		}
	}
	TestTrue(TEXT("write tool_matrix_last.json"), FFileHelper::SaveStringToFile(OutStr, *OutPath));
	...
	for (const TSharedPtr<FJsonValue>& V : ContractViolations)
	{
		...
		if (V.IsValid() && V->TryGetObject(O) && O->IsValid())
		{
			...
			AddError(FString::Printf(TEXT("CatalogMatrix contract: %s — %s"), *Tid, *Reason));
		}
	}
